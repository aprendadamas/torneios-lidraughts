...
def extract_tournaments(html_content):
    ...
    for a in soup.select('a[href^="/tournament/"]'):
        ...
        if "Brazilian" in text:
            ...
            # Remover partes desnecessárias do nome
            name = text
            # Remover "• Brazilian • Rated •" e similares
            name = re.sub(r'•.*
...
def generate_html(tournaments):
    ...
    # Se não tivermos torneios organizados por data mas tivermos o HTML antigo,
    ...
    if not existing_tournaments_by_date and existing_html and "tournament-list" in existing_html:
        ...
        if old_tournaments:
            # Adicionar todos os torneios antigos à data atual
        ...
        if today_str in existing_tournaments_by_date:
            ...
            for new_t in new_tournaments:
                if new_t["url"] not in existing_urls:
                    ...
                    print(f"Torneio já existe, pulando: {new_t['name']}")
        else:
...
, '', name).strip()
            ...
            # Se ainda não tiver nome, usar o ID do torneio
            if not name or name == "-":
    ...
    # Como não temos o número de participantes, vamos pegar os primeiros TOP_TOURNAMENTS
    ...
    if tournaments:
        print(f"Selecionados {len(tournaments)} torneios únicos")
        return tournaments
    return []
