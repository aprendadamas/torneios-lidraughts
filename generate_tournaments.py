...
def extract_brazilian_tournaments(html_content):
    ...
    for link in tournament_rows:
        ...
        if 'Brazilian' in row_text:
            ...
            parent = link.parent
            numbers_found = False
            while parent and not numbers_found:
                # Procurar por números no texto do parent
...
def extract_existing_tournaments_by_date(html_content):
    ...
    if day_sections:
        print(f"   Formato com day-sections detectado ({len(day_sections)} seções)")
        for section in day_sections:
            # Extrair a data do h2
            date_h2 = section.find('h2')
            if date_h2:
                date_text = date_h2.get_text(strip=True)
                print(f"   Processando seção: {date_text}")
                ...
                if date_match:
                    ...
                    print(f"      ⚠️ Não foi possível extrair data de: {date_text}")
                    continue
                ...
                print(f"      Encontrados {tournament_count} torneios")
    else:
...
"""
        ...
        for date_str in sorted_dates:
            ...
            # Pular esta data se não houver torneios válidos
