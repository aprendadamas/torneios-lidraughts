#!/usr/bin/env python3
"""
Solve Exercise 11 - CORRECTED - Force d6 to move
"""
...
def find_combinations_from_first_move(fen, first_move_pdn, max_depth=9):
    """Find all combinations starting with a specific first move"""
    board = Board(variant="brazilian", fen=fen)

    # Make the first move
    first_move_obj = None
    for move in board.legal_moves():
        if move.pdn_move == first_move_pdn:
            first_move_obj = move
            break

    if not first_move_obj:
        print(f"First move {first_move_pdn} not found!")
        return []

    board.push(first_move_obj)
    print(f"Starting search from position after: {first_move_pdn}")
    print(board)
    print()
    ...
    search(board, [first_move_pdn], max_depth)
...
def main():
    ...
    print("The key insight: f4-e5 FORCES d6 to capture (mandatory capture rule)")
    print("This removes d6 from blocking the combination!")
    print()

    # Search combinations starting with f4-e5
    first_move = "15-19"  # f4-e5 in PDN
    combinations = find_combinations_from_first_move(fen, first_move, max_depth=9)

    if not combinations:
        print("\nNo decisive combinations found starting with f4-e5!")
        return

    print(f"\nFound {len(combinations)} decisive combinations starting with f4-e5.")
    print(f"\nTop 5 best combinations:\n")

    for i, (score, moves, material) in enumerate(combinations[:5], 1):
